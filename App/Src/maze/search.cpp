  ...
  do{
    ...
    switch(route_[r_cnt_++]){         //route配列によって進行を決定。経路カウンタを進める
      ...
      //Objects::led_->On();
        ...
        //前壁をあるときだけけつあて
        if(wall_->get_val_ref()->dir[static_cast<int>(state::Wall::DIR::F)] == 1){
          core_->Spin();
          core_->Stop();
          core_->Ketsu();
          wait_->Ms(100);
          ...
          core_->Spin();  //180度回転
          core_->Stop();
          core_->Straight(consts::software::HALF_BLOCK,0,spd_);  //半区画分加速しながら走行する
        }
